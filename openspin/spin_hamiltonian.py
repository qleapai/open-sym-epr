...
class NuclearSpec:
    ...
    label: str = ""
    gn: float = 0.0  # nuclear g-factor (for nuclear Zeeman / ENDOR); 0 -> ignored
...
NMAGN_MHZ_PER_T = 7.622593285  # nuclear magneton over h, MHz/T

# Nuclear g-factors for common isotopes (gn = mu/(I mu_N)); used for nuclear Zeeman.
NUCLEAR_G_FACTORS = {
    "1H": 5.5857, "2H": 0.8574, "13C": 1.4048, "14N": 0.4038, "15N": -0.5664,
    "19F": 5.2578, "31P": 2.2632, "27Al": 1.4566, "51V": 1.4711, "55Mn": 1.3819,
    "63Cu": 1.4824, "65Cu": 1.5877,
}
...
def build_nuclear_zeeman_direction(system: SpinSystem, direction: np.ndarray) -> np.ndarray:
    """Field-linear nuclear-Zeeman operator (MHz per Tesla) summed over nuclei.

    H_nz(B) = -B[T] * sum_k gn_k mu_N (d . I_k).  Needed for ENDOR, where the line
    positions depend on the nuclear Larmor frequency; negligible for cw-EPR.
    """
    l, m, n = direction
    nbig = system.hilbert_dim
    H = np.zeros((nbig, nbig), dtype=complex)
    for k, nuc in enumerate(system.nuclei):
        gn = nuc.gn or NUCLEAR_G_FACTORS.get(_iso_key(nuc.label), 0.0)
        if gn == 0.0:
            continue
        ix, iy, iz = _nuclear_ops(system, k)
        H = H - gn * NMAGN_MHZ_PER_T * (l * ix + m * iy + n * iz)
    return H


def _iso_key(label: str) -> str:
    """Best-effort isotope key from a nucleus label (e.g. '14N nitroxide' -> '14N')."""
    import re
